      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        >
                ...
                {product.images.map((img, idx) => (
                  <button 
                    ...
                    className={cn(
                      ...
                      activeImage === idx ? "border-[#B08D57]" : "border-transparent opacity-60 hover:opacity-100"
                    )}
                  >
          ...
          <div className="w-full md:w-1/2 p-6 sm:p-10 flex flex-col overflow-y-auto hide-scrollbar">
            ...
            <h2 className="text-2xl font-serif text-[#2B2B2B] mb-2">{product.name}</h2>
            
            <div className="flex items-center gap-2 mb-4">
              <span className="text-[#B08D57] text-sm">★★★★★</span>
              <span className="text-sm text-[#8C8C8C]">{product.rating} ({product.reviewsCount} Reviews)</span>
            </div>

            <div className="flex items-center gap-3 mb-6">
              <span className="text-xl font-bold text-[#2B2B2B]">EGP {product.price}</span>
              {product.oldPrice && (
                <span className="text-sm text-[#8C8C8C] line-through">EGP {product.oldPrice}</span>
              )}
            </div>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-3">
                ...
                <button className="text-xs text-[#8C8C8C] underline hover:text-[#B08D57]">Size Guide</button>
              </div>
